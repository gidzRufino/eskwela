<div class="row">
    <div class="col-lg-12">
        ...
        <div class="pull-right" >
           <select onclick="getEmployeeAccess(this.value)" tabindex="-1" id="inputTeacher" style="width:300px; font-size:15px;" class="span2">
               <option>Select Employees Here</option>
               <?php 
                         foreach ($employees->result() as $e)
                      {   
                     ?>                        
               <option value="<?php echo $e->user_id; ?>"><?php echo strtoupper($e->firstname.' '. $e->lastname); ?></option>
                   <?php }?>
           </select>
...
//            document.getElementById('dbAccess').value = (dumpInArray().join(","));
        ...
        $('.select-roles').click(function () {
          // $('#result').html(dumpInArray().join(","));
           ...
           var url = "<?php echo base_url().'college/settings/saveAccess/'?>"// the script where you handle the form input.
...
//                       $('#alert-info').fadeOut(5000);
    ...
    function getEmployeeAccess(value)
    {
       
     var url = "<?php echo base_url().'college/settings/getEmployeeAccess/'?>"+value// the script where you handle the form input.

        $.ajax({
               type: "POST",
               url: url,
               data: "employee_id="+value+'&csrf_test_name='+$.cookie('csrf_cookie_name'), // serializes the form's elements.
               success: function(data)
               {
