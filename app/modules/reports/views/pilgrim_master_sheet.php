...
class MYPDF extends Pdf {
    ...
    public function Header() {
        ...
        $this->SetTitle($settings->short_name . ' Master Sheet ');
        $image_file = K_PATH_IMAGES . '/pilgrim.jpg';
        $this->Image($image_file, 150, 7, 18, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);

        $image_file = K_PATH_IMAGES . '/uccp.jpg';
        $this->Image($image_file, 233, 7, 15, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        $this->Ln(5);
...
//    unset($grd);
    foreach ($subject_ids as $sub) {
        ...
        $genAve += ($finalGrade->row()->final_rating<=70?70:$finalGrade->row()->final_rating);
        $numSubs++;
...
//                $pdf->MultiCell(18, 3,$mfg,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//            else:
        if ($finalGrade->row()->final_rating != 0):
            $pdf->MultiCell(18, 3, ($finalGrade->row()->final_rating<=70?70:$finalGrade->row()->final_rating), 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        else:
...
//                endif;
//                if($singleSub->subject_id==18):
//                    $pdf->MultiCell(18, 3,$ffg,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
//                else:
        if ($finalGrade->row()->final_rating != 0):
            $pdf->MultiCell(18, 3, ($finalGrade->row()->final_rating<=70?70:$finalGrade->row()->final_rating), 1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        else:
