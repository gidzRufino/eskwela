...
class accountingsystem_model extends CI_Model {
    ...
    function deleteASTrans($je_num)
    {
        $this->db->where('as_je_num', $je_num);
        if($this->db->delete('as_transactions')):
            return TRUE;
        else:
            return FALSE;
        endif;
    ...
    {
        $this->db->join('as_chart_of_accounts','as_transactions.as_coa_id = as_chart_of_accounts.coa_id','left');
        $this->db->where("as_trans_date between '" . $from . "' and'" . $to . "'");
        $q = $this->db->get('as_transactions');
        return $q->result();
    }
            
    function getTransactionPerType($type)
    {
        $this->db->where('as_trans_type', $type);
        $this->db->join('as_chart_of_accounts','as_transactions.as_coa_id = as_chart_of_accounts.coa_id','left');
        $q = $this->db->get('as_transactions');
        return $q->result();
    }
    ...
    function saveASTransaction($column)
    {
        $result = $this->db->insert_batch('as_transactions', $column);
        if($result):
            return TRUE;
        else:
            return FALSE;
