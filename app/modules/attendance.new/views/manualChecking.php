    ...
    function saveAttendance(st_id, rfid)
       {
          //; alert(rfid)
            ...
            $.ajax({
                   type: "POST",
                   url: url,
                   data: "id="+rfid+"&section_id="+<?php echo $section_id; ?>+'&st_id='+st_id+'&date='+date+'&csrf_test_name='+$.cookie('csrf_cookie_name'), // serializes the form's elements.
                   beforeSend: function() {
                    if(st_id==0){
    ...
    function deleteAttendance(id, st_id)
     {
         var url = "<?php echo base_url().'attendance/deleteAttendance' ?>"; // the script where you handle the form input.
            ...
            $.ajax({
                   type: "POST",
                   url: url,
                   data: "att_id="+id+'&csrf_test_name='+$.cookie('csrf_cookie_name'), // serializes the form's elements.
                   success: function(data)
                   {
                       //$("form#quoteForm")[0].reset()
                        location.reload();
