...
//                  use to update finance details
    ...
    function getAllStudentsByGender($section_id = null, $gender = null, $status = NULL, $year = NULL, $strand = NULL) {
        $result = $this->get_registrar_model->getAllStudentsByGender($section_id, $gender, $status, $year, $strand);
        return $result;
        //echo $result->num_rows();
    }
    ...
    function getLatestIdNum($level_id) {
        $this->load->model('college/enrollment_model');
        ...
        echo json_encode(array('status' => TRUE, 'id' => $idNumber, 'deptCode' => $deptCode->deptCode));
    }
...
//    }
    ...
    public function saveAdmission()
    {
        ...
        $userExist = $this->registrar_model->checkName($this->input->post('inputCLastName'),$this->input->post('inputCFirstName'),$this->input->post('inputCMiddleName'), $school_year);
        
        if(!$userExist){
            $idNumber = Modules::run('opl/enrollment/generateID', $school_year);
            
            $st_id = $school_year.$semester.'2'.$idNumber;

            $generatedCode = $this->eskwela->generateRandNum();
            $this->registrar_model->saveTempID($st_id, $generatedCode,$school_year);

            $this->registrar_model->setBarangay($this->input->post('inputBarangay'), $generatedCode, $school_year);
            ...
            $this->registrar_model->setAddress($add, $school_year);
            ...
            $this->registrar_model->saveProfile($items, $school_year);

            //saves the basic contact
            $this->registrar_model->setContacts($this->input->post('inputPhone'), $this->input->post('inputEmail'), $generatedCode, $school_year);
            ...
            $this->registrar_model->setCollegeInfo($st_id, $generatedCode, $this->input->post('getCourse'),  $this->input->post('inputYear'), $enDate, $school_year, $this->input->post('inputSemester'), ($this->input->post('inputSLA')==""?"(No Entry)":$this->input->post('inputSLA')), ($this->input->post('inputAddressSLA')==""?"":$this->input->post('inputAddressSLA')));


            $f_occupation_id = $this->registrar_model->saveOccupation($this->input->post('inputF_occ'), $school_year);
            $m_occupation_id = $this->registrar_model->saveOccupation($this->input->post('inputM_occ'), $school_year);
                ...
                $this->registrar_model->saveParentDetails($parentDetails, $school_year);

                 //Medical information
    //            $this->registrar_model->saveMed(
    ...
    //                $generatedCode, $school_year );
            ...
            $result = json_decode(Modules::run('college/enrollment/sendConfirmation', $this->post('inputPhone'), $message));
            ...
            while($resultStatus!=1):
                ...
                $result = json_decode(Modules::run('college/enrollment/sendConfirmation', $this->post('inputPhone'), $message));
                if($try==10):
                   // $this->registrar_model->approveLoad(base64_decode($st_id), $school_year, 0);
//                    $statusArray = array(
//                        'status'    => true,    
//                        'msgs'      => $msg
//                    );
...
//        echo json_encode($oldData);
    ...
    public function saveOnlineRO() {
        ...
        if (!$isEnrolled):
            ...
            if ($semester == 0):
                $profile = $this->get_registrar_model->getPreviousRecord('profile', 'user_id', $oldDetails->user_id, ($school_year - 1), $settings);

                $profile_students = $this->get_registrar_model->getPreviousRecord('profile_students', 'user_id', $profile->user_id, ($school_year - 1), $settings);
                $profile_address = $this->get_registrar_model->getPreviousRecord('profile_address_info', 'address_id', $profile->add_id, ($school_year - 1), $settings);
                $profile_contact = $this->get_registrar_model->getPreviousRecord('profile_contact_details', 'contact_id', $profile->contact_id, ($school_year - 1), $settings);
                $profile_parents = $this->get_registrar_model->getPreviousRecord('profile_parent', 'u_id', $profile->user_id, ($school_year - 1), $settings);

                $this->get_registrar_model->insertData($profile, 'profile', 'user_id', $profile->user_id, $sy);
...
//                'section_id' => ($section->row() ? $section->row()->s_id : 0),
            ...
            $ro_id = $this->get_registrar_model->saveBasicRO($details, $sy);
            if (!$ro_id):
                echo 'Sorry, something went wrong, Please Try Again';
            else:
                switch ($semester):
    ...
    function cp_profile_parent() {
        $this->db = $this->eskwela->db(2019);
        $past = $this->db->get('profile_parent')->result();

        foreach ($past as $p):
            $this->db = $this->eskwela->db(2020);
            $this->db->where('u_id', $p->u_id);
            $present = $this->db->get('profile_parent');
//
...
//                return FALSE;
                ...
                $data = array(
                    ...
                    'u_id' => ($p->u_id != '' ? $p->u_id : ''),
                    'f_lastname' => ($p->f_lastname != '' ? $p->f_lastname : ''),
                    'f_firstname' => ($p->f_firstname != '' ? $p->f_firstname : ''),
                    'f_middlename' => ($p->f_middlename != '' ? $p->f_middlename : ''),
                    'f_mobile' => ($p->f_mobile != '' ? $p->f_mobile : ''),
                    'f_add_id' => ($p->f_add_id != '' ? $p->f_add_id : ''),
                    'f_occ' => ($p->f_occ != '' ? $p->f_occ : ''),
                    'f_educ' => ($p->f_educ != '' ? $p->f_educ : ''),
                    'f_office_name' => ($p->f_office_name != '' ? $p->f_office_name : ''),
                    'f_office_address_id' => ($p->f_office_address_id != '' ? $p->f_office_address_id : ''),
                    'm_lastname' => ($p->m_lastname != '' ? $p->m_lastname : ''),
                    'm_firstname' => ($p->m_firstname != '' ? $p->m_firstname : ''),
                    'm_middlename' => ($p->m_middlename != '' ? $p->m_middlename : ''),
                    'm_mobile' => ($p->m_mobile != '' ? $p->m_mobile : ''),
                    'm_add_id' => ($p->m_add_id != '' ? $p->m_add_id : ''),
                    'm_occ' => ($p->m_occ != '' ? $p->m_occ : ''),
                    'm_educ' => ($p->m_educ != '' ? $p->m_educ : ''),
                    'm_office_name' => ($p->m_office_name != '' ? $p->m_office_name : ''),
                    'm_office_address_id' => ($p->m_office_address_id != '' ? $p->m_office_address_id : ''),
                    'ice_name' => ($p->ice_name != '' ? $p->ice_name : ''),
                    'ice_contact' => ($p->ice_contact != '' ? $p->ice_contact : ''),
                    'guardian' => ($p->guardian != '' ? $p->guardian : '')
                );
            echo $p->u_id . '<br>';
                $this->db = $this->eskwela->db(2020);
    ...
    function updateAttLog(){
        $this->db->where('date', '2020-10-16');
        $q = $this->db->get('attendance_sheet_oct')->result();
        foreach ($q as $p):
            echo $p->u_rfid . '<br>';
            $data = array(
            ...
            $this->db->insert('attendance_log_book', $data);
        endforeach;
    }
}
