...
//            endif;
    ...
    function getAllStudentsBasicInfoByGender($section_id = null, $gender = null, $status = NULL, $year = NULL, $strand = NULL) {
        $result = $this->get_registrar_model->getAllStudentsBasicInfoByGender($section_id, $gender, $status, $year, $strand);
        return $result;
        //echo $result->num_rows();
    }
    ...
    public function saveRO() {
        ...
        $profile_parents = $this->get_registrar_model->getPreviousRecord('profile_parent', 'u_id', $profile->user_id, $school_year, $settings);
        ...
        $sy = $school_year + 1;
        if (!empty($profile)):
            $this->get_registrar_model->insertData($profile, 'profile', NULL, NULL, $sy);
            $this->get_registrar_model->insertData($profile_students, 'profile_students', NULL, NULL, $sy);
            $this->get_registrar_model->insertData($profile_parents, 'profile_parent', NULL, NULL, $sy);
//            $parent_details = array(
//                'u_id' => $profile->user_id,
//                'f_lastname' => $profile_parents->father_id,
//                'f_firstname' => $profile_parents->mother_id,
//                'f_middlename' => $profile_parents->f_office_name,
//                'f_mobile' => $profile_parents->f_office_address_id,
...
//            );
//
//            $this->get_registrar_model->insertData($parent_details, 'profile_parent', NULL, NULL, $sy);
//
//            $this->get_registrar_model->insertData($m_profile, 'profile', NULL, NULL, $sy);
