...
foreach($sales as $s): 
    ...
    if($total!=0):
        $overAll += $total;
        $pdf->MultiCell(16, 3, '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(30, 3, date('F d, Y', strtotime($s->transaction_date)),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(60, 3, $s->canteen_item_name,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(20, 3, $item->item_quantity,1, 'R', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(30, 3, number_format($total, 2, ".", ','),1, 'R', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->ln();
         if($z==34):
             $z=0;
            $pdf->AddPage();
...
endforeach;
        ...
        $pdf->setCellPaddings(1, 1, 1, 1);
        $pdf->MultiCell(16, 3, '',0, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        $pdf->MultiCell(110, 3, 'TOTAL','BL', 'L', 0, 0, '', '', true, 0, false, true, 10, 'M');
        ...
        $pdf->MultiCell(50, 3, 'Cafeteria Manager','T', 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
