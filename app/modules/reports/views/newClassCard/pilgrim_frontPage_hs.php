...
foreach($students->result() as $mStudent):
    ...
    $pdf->MultiCell(22, 5, 'Grade Level :',0, 'R', 0, 0, '', '', true, 0, false, true, 5, 'T');
    $pdf->SetFont('helvetica', 'b', 8);
    $pdf->MultiCell(37, 5, $mStudent->level.' - '.$section->section,'B', 'L', 0, 0, '', '', true, 0, false, true, 5, 'T');
    $pdf->Ln(8);
    $pdf->SetX($x);
    $pdf->SetFont('helvetica', 'b', 9);
        ...
        foreach($subject_ids as $sub):
          $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
            ...
            if($finalGrade->row()->final_rating!=""):
                ...
                $pdf->MultiCell(33, 5, ($finalGrade->row()->final_rating<=70?70:$finalGrade->row()->final_rating),1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'T');
            else:
