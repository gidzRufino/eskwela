...
?>
        ...
        <table class="table table-striped">
            ...
            <?php 
                ...
                $coreSubs = Modules::run('subjectmanagement/getSHSubjects', $student->grade_id, $sem, $student->strnd_id, 1);  
                $i=0;
                foreach($coreSubs as $sub):
                    $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sh_sub_id);
                    $finalGrade = Modules::run('gradingsystem/getFinalGrade', $student->st_id, $singleSub->subject_id, $term, $sy);   
                    ?>
                    <tr>
                ...
                <?php        
                $appliedSubs = Modules::run('subjectmanagement/getSHSubjects', $student->grade_id, $sem, $student->strnd_id); 
                $i=0;
       ...
       ?>
        ...
        ?>
            ...
            <div class="col-lg-6">
                <?php 
                $sprDetails = Modules::run('sf10/getSPRrec',$student->uid, $sy, $sem, $student->grade_id );
                echo Modules::run('sf10/getAttendanceDetailsSH', $sprDetails->spr_id, $sprDetails->school_year, $sem); 
                ?>
            </div>    
            <div class="col-lg-6">
                <h5 class="col-lg-6">Learner's Observed Values</h5>
                <br />
                <hr />
...
<?php 
$data['strand'] = $student->strnd_id;
$this->load->view('reportCardPreview', $data); 

echo Modules::run('sf10/attendanceManualOveride', base64_encode($student->st_id), $sy, $sem, FALSE, TRUE);
?>
<script type="text/javascript">
    ...
    {
        var url = "<?php echo base_url().'reports/cardReview/'?>"+st_id+'/'+sy+'/'+term+'/<?php echo $student->strnd_id; ?>';
        $.ajax({
               type: "GET",
               url: url,
    ...
    function printCard(st_id, sy, term, strand)
    {
        ...
        var url = "<?php echo base_url().'reports/printReportCard/'?>"+st_id+'/'+sy+'/'+term+'/'+strand+'/'+admittedTo+'/'+dateAdmitted;
