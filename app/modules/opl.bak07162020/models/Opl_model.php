...
/*    end of grading system  */
    ...
    function getPost($id)
    {
        return $this->db->join('profile_employee', 'opl_posts.op_owner_id = profile_employee.employee_id')
                ->join('profile', 'profile_employee.user_id = profile.user_id')
                ->where('op_owner_id = "'.$id.'" OR (op_target_type != 3 AND op_target_type != 2 AND op_target_type IS NOT NULL)')
