...
<body style="overflow-y: hidden;">
    ...
    <script type="text/javascript">
            ...
            if (previousId == ID) {
                ...
                $.ajax({
                    ...
                    success: function (data)
                    {
                        if(data.status)
                        {
                            sendNotification(data.textmsg, data.parent_id);
                        }
                        if (data.status) {
        ...
        function sendNotification(msg, parent_id)
        {
            var url = "<?php echo base_url() . 'notification_system/sendPushNotification/' ?>"+msg+'/'+parent_id; // the script where you handle the form input.
            $.ajax({
                type: "POST",
                url: url,
                data: 'status=1' + '&csrf_test_name=' + $.cookie('csrf_cookie_name'), // serializes the form's elements.
