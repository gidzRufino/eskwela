<?php
$student = Modules::run('registrar/getSingleStudent', base64_decode($st_id), $this->session->userdata('school_year'), $semester);
...
<div id="otherMenu">
    <ul class="dropdown-menu" role="menu">
        <li onclick="$('#editFinTransaction').modal('show')" class="pointer text-danger"><i class="fa fa-edit fa-fw"></i>Edit TRANSACTION</li>
            ...
            success: function (response) {
                ...
                <?php if($finSettings->print_receipts):?>
                    if($('#printOR').is(':checked')){
                        var url = '<?php echo base_url('finance/printOR/'.base64_encode($student->uid).'/')?>'+or_num+'/'+(transType==0?'Cash':'Cheque')+'/'+$('#ptAmountTendered').val();
                        window.open(url, '_blank');    
                    }
                <?php endif; ?>
