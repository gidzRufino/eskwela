...
foreach($getTopTen as $tt=>$gt):
    ...
    if($x<11):
        ...
        $pdf->MultiCell(20, 3, number_format(round($finalTotal/$fa,3),3),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        //$pdf->MultiCell(20, 3, round($finalAverage/$fa, 3),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
        if(number_format(round($finalTotal/$fa,3),3)>=98):
            $rank = 'With Highest Honors';
        elseif(number_format(round($finalTotal/$fa,3),3)<=97.99 && number_format(round($finalTotal/$fa,3),3) >= 95):
            $rank = 'With High Honors';
        elseif(number_format(round($finalTotal/$fa,3),3)<= 94.99 && number_format(round($finalTotal/$fa,3),3) >= 90):
            $rank = 'With Honors';
        else:
            $rank = '';
        endif;
