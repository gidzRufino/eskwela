...
class gradingsystem_model extends CI_Model {
    ...
    function getbhTransmuted(){
        return $this->db->get('gs_final_bh_rate_cust_trans')->result();
    }
    ...
    function getTotalScoreByStudent($student_id, $qcode, $term, $subject_id, $faculty_id,$section) {
        $this->db->select('*');
        $this->db->select('gs_raw_score.assess_id as as_id');
        ...
        //$this->db->where('gs_assessment.section_id',$section);
        $this->db->where('st_id', $student_id);
        $this->db->where('gs_assessment.quiz_cat', $qcode);
        $this->db->where('gs_assessment.faculty_id', $faculty_id);
        $this->db->order_by('raw_score', 'desc');
        $query = $this->db->get();
        return $query;
    }
...
//        return $query->row();
    ...
    function getFinalCardByLevel($st_id, $level, $school_year) {
        if ($school_year != NULL):
            $this->db = $this->eskwela->db($school_year);
        endif;
