...
<!--    <button class="btn btn-xs btn-success pull-right" st-id="<?php echo base64_encode($student->st_id); ?>" onclick="readyPrint(this)">Print Admission Form</button>                        
    <script>
    ...
    <table class="table table-striped table-bordered table-responsive" id="">
        ...
        if ($this->session->userdata('position_id') == 1 ||
                ...
                $this->session->position == 'Enrollment Officer' ||
...
Hi <?php echo ucwords(strtolower($student->firstname)) ?>
            ...
            <input type="text" id="mobileNo" value="" />
...
function sendSMS() {
        var smsDetails = $('#smsDetails').val();
        var mobileNum = $('#mobileNo').val();
        var url = '<?php echo base_url() . 'college/enrollment/sendSMS/' ?>' + mobileNum;

        $.ajax({
            type: 'POST',
            data: 'msg=' + smsDetails + '&csrf_test_name=' + $.cookie('csrf_cookie_name'),
            url: url,
            beforeSend: function ()
            {
                $('#loadingModal').modal('show');
    ...
    {
        var base = $('#base').val();
        var url = base + 'college/enrollment/removeBasicEnrollmentDetails/'; // the script where you handle the form input.
        ...
        if (con == true) {
            ...
            $.ajax({
                ...
                {
                    alert(data)
                    document.location = base + 'college/enrollment/monitor/0/2020';
                }
            });
