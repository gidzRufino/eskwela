<?php 
    ...
    $acadSubjects = Modules::run('college/gradingsystem/getRecordedGrade', $details->st_id, NULL, NULL, NULL,$school_year,NULL,1);
?>
    ...
    <table class="table table-hover">
        ...
        <tr>        
         ...
         foreach($term->result() as $t):  
                $grade = Modules::run('college/gradingsystem/getRecordedGrade', $details->st_id, NULL, $t->gst_id, NULL, $school_year, $as->sub_id, 1);
                //print_r($grade->row());
                if($this->session->is_admin):
            ?>
                    <input type="hidden" id="raw_<?php echo $as->sub_id.'_'.$ac->gsc_id.'_'.$t->gst_id ?>" value="<?php echo $grade->row()->grade; ?>" />
            <?php
                endif;
         ...
         ?>
            <td
                id="td_<?php echo $as->sub_id.$t->gst_id.$as->semester ?>"
                onclick="getInitialGrade('<?php echo $as->sub_id ?>', '<?php echo $details->st_id ?>', '<?php echo $t->gst_id ?>','<?php echo $as->semester ?>','<?php echo $as->school_year ?>','<?php echo $subject->sub_code ?>','<?php echo $t->gst_term ?>'  )"
                style="width:10%;" class="text-center grade_<?php echo $as->sub_id ?>"><strong><?php echo ($partialGrade!=0? Modules::run('college/gradingsystem/getTransmutation', number_format($partialGrade,2,'.',',')):'') ?></strong></td>
        ...
        ?>
            ...
            <td class="text-right">
                    <button title="Overwrite Grades" onclick="$('.grade_<?php echo $as->sub_id ?>').addClass('editable')" style="margin: 10px;" class="btn btn-warning btn-xs pull-right"><i class="fa fa-edit"></i></button>
...
<?php endif;  ?>
<div id="overWriteModal"  style="width:40%; margin: 50px auto;"  class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="panel panel-red" style='width:100%;'>
        <div class="panel-heading">
        ...
        <div class="panel-body">
            <div class="form-group">
                <label class="control-label">Class Standing:</label>
                <input style="margin-bottom:0;" class="form-control"    name="classStanding" type="text" id="classStanding" required>
            </div>
            <div class="form-group">
...
<script type="text/javascript">
    ...
    function getInitialGrade(sub_id, st_id, term, sem, sy, subject, termName)
    {
        if($('#td_'+sub_id+term+sem).hasClass('editable'))
        {
            $('#classStanding').val($('#raw_'+sub_id+'_1_'+term).val());
            $('#exam').val($('#raw_'+sub_id+'_2_'+term).val());
