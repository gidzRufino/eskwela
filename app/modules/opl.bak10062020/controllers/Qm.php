...
class Qm extends MX_Controller {
    ...
    {
        $ans = explode("|", $this->qm_model->checkAnswer($q_code, $school_year)->qq_answer);
        $checked = FALSE;
        if(count($ans) == 1):
            $ans = $this->qm_model->checkAnswer($q_code, $school_year)->qq_answer;
           // return (trim(strtolower($answer))== trim(strtolower($ans)));
            if(trim(strtolower($answer))== trim(strtolower($ans))):
                    $checked = TRUE;
                endif;
                return $checked;
        else:
            foreach($ans AS $an):
                if(trim(strtolower($answer))== trim(strtolower($an))):
                    $checked = TRUE;
                    break;
                endif;
            endforeach;
