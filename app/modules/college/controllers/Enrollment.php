...
class Enrollment extends MX_Controller {
    ...
    public function saveReligion(){
        $response = $this->enrollment_model->addReligion($this->eskwela->codeCheck('religion', 'rel_id', $this->eskwela->code()), $this->post('religionName'));
            ...
            switch($type):
                case 3:
                    ...
                    (
                        ...
                        'max_size'      =>  100,
                        'file_name'     =>  'gc_'.$this->eskwela->codeCheck('padala_centers', 'file_name', 'gc_'.$this->eskwela->code())
                    );
                    $pcName = 'GCash';
                    $pcShortName = 'gc';
                break;
                default:
                    ...
                    (
                        ...
                        'allowed_types' =>  'gif|jpg|png',
                        'max_size'      =>  100,
                        'file_name'     =>  $this->post('pcShortName').'_'.$this->eskwela->codeCheck('padala_centers', 'file_name', $this->post('pcShortName').'_'.$this->eskwela->code())
                    );
                break;
            endswitch;
    ...
    {   
        ...
        if($response == TRUE)
        {
            echo json_encode(array('type' => 1, 'message' => 'Successfully added payment center'));
        }
...
//    }
    ...
    public function saveBasicEdAdmission()
    {
        ...
        if(!$userExist){
            ...
            $generatedCode = $this->eskwela->codeCheck('profile_temp_id', 'temp_id', $this->eskwela->code());
...
//                    );
    ...
    {       
            ...
            if (!$this->upload->do_upload()) {
                // $data['error'] = $this->upload->display_errors();
                $data['success'] = 0;
                $data['msg'] = str_ireplace('<p>', '', $this->upload->display_errors());
                $data['msg'] = str_ireplace('</p>', '', $data['msg']);
                ...
                if($this->enrollment_model->savePaymentReceipt(base64_decode($st_id),$link, $school_year, $semester)):
                    $data['success'] = 1;
                    $data['msg'] = 'Thank you for uploading. We will just notify you once your payment is confirm.';
                    echo json_encode($data);
                    if($this->session->department==4):
                        $student = $this->enrollment_model->getStudentDetails(base64_decode($st_id), $semester, $school_year);
    ...
    public function sendFinanceRemarks($isBasicEd = NULL)
    {
        ...
        $remarkDetails = array(
            'fr_id'         => $this->eskwela->codeCheck('c_finance_remarks', 'fr_id', $this->eskwela->code()),
            'fr_to'         => $st_id,
            'fr_remarks'    => $remarks,
    ...
    public function verifyOTP() {
        ...
        $admStat = $this->enrollment_model->checkStAccStat($id, 'profile_students_admission', 'st_id');
        $usrAcc = $this->enrollment_model->checkStAccStat($id, 'ua_students', 'uname');
        ...
        if ($password_matched) {
            if ($admStat->row()->status == 1 && $usrAcc->num_rows() > 0):
                echo json_encode(array('status' => TRUE, 'url' => base_url('college/enrollment/entrance/' . base64_encode($id) . '/' . $department . '/MA==/' . base64_encode($usrAcc->row()->secret_key) . '/' . base64_encode($id . $department))));
            else:
                if ($department == 5): //college
                    ...
                    $school_year = ($semester == 3 ? ($settings->school_year - 1) : $settings->school_year);
                    $isCollege = FALSE;
                    $isOld = $this->enrollment_model->isPreviouslyEnrolled($id, ($school_year - 1), $isCollege);
                ...
                if ($admStat->row()->status == 1):
                    echo json_encode(array('status' => TRUE, 'url' => base_url('opl')));
                else:
    ...
    public function requestEntry() {
        ...
        $admStat = $this->enrollment_model->checkStAccStat($idNumber, 'profile_students_admission', 'st_id');
        $usrAcc = $this->enrollment_model->checkStAccStat($idNumber, 'ua_students', 'uname');
        $lrnStat = $this->enrollment_model->checkStAccStat($idNumber, 'profile_students', 'lrn');
        if ($result->status):
            if ($admStat->row()->status == 1 && $usrAcc->num_rows() > 0):
                $jsonResponse = array(
                    'status' => TRUE,
                    ...
                    'contact_num' => '1234',
                    'url' => base_url('college/enrollment/entrance/' . base64_encode($idNumber) . '/' . $department . '/' . base64_encode($this->post('semester')) . '/' . base64_encode($usrAcc->row()->secret_key))
                );

                echo json_encode($jsonResponse);
            elseif($lrnStat->num_rows() != 0):
                $idNumber = $lrnStat->row()->st_id;
                $usrAcc = $this->enrollment_model->checkStAccStat($idNumber, 'ua_students', 'uname');
                $jsonResponse = array(
                    'status' => TRUE,
                    'option' => $admStat->row()->status,
    ...
    public function entrance($st_id = NULL, $department = NULL, $semester = NULL, $pass = NULL, $changePW = NULL) {
        if ($this->session->is_logged_in):
    ...
    public function sendConfirmation($number=NULL, $msg=NULL) {
        ...
        return json_encode(array('number' => $number, 'msg' => $result, 'status' => $stat));
    }
...
//    
    ...
    public function saveLoadedSubjects()
    {
        ...
        foreach ($data as $d):
            ...
            $details = array(
                'cl_id'         => $this->eskwela->codeCheck('subjects', 'cl_id', $this->eskwela->code()),
    ...
    function removeEnrollmentDetails()
    {
        ...
        if($this->enrollment_model->removeEnDetails($school_year, $admission_id, $segment)):
...
//            print_r($fType . '-' . $stid .'-'. $_FILES['userfile']['name']);
    ...
    function isOld($stid, $semester, $school_year = NULL){
        return $this->enrollment_model->isOld($stid, $semester, $school_year);
    }
    ...
    function verifyPassword() {
        ...
        if ($res):
            ...
            $this->session->set_userdata($userData);
            $this->enrollment_model->getInside($stid, md5($pass));
