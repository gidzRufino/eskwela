...
foreach($male->result() as $s){
    ...
    foreach($subject_ids as $sub){  
          ...
          $assessment = Modules::run('gradingsystem/getPartialAssessment', $s->st_id, segment_3, $sub->sub_id, segment_5);
          if(!empty($assessment)):
            switch($gs_settings->gs_used):
                case 1:
                     $ass_gr = $assessment->$term;
                break;
                case 2:
                     $ass_gr = Modules::run('gradingsystem/new_gs/getTransmutation', $assessment->$term);
                break;    
            endswitch;
    ...
    $pdf->MultiCell(5, 3, '',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, round($finalAssessment/count($subject_ids), 3),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if(round($finalAssessment/count($subject_ids), 2)>=75):
...
foreach($female->result() as $fem){
    ...
    foreach($subject_ids as $sub){  
          $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
          $assessment = Modules::run('gradingsystem/getPartialAssessment', $fem->st_id, segment_3, $sub->sub_id, segment_5);
          if(!empty($assessment)):
    ...
    $pdf->MultiCell(5, 3, '',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, round($finalAssessment/(count($subject_ids)-1), 2),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if(round($finalAssessment/(count($subject_ids)-1), 2)>=75):
        $pdf->MultiCell(35, 3, 'PASSED',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    else:
        $pdf->MultiCell(35, 3, 'FAILED',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    endif;
