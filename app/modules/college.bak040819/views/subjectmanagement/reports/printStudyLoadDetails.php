...
$pdf->MultiCell(42, 0, $this->session->userdata('school_year').' - '. $next,0, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(68, 0, 'Subject',1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(10, 0, 'Unit/s',1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(15, 0, 'Day',1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
$pdf->MultiCell(20, 0, 'Time',1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
foreach ($loadedSubject as $sl):
    ...
    $pdf->MultiCell(68, 0, $sl->s_desc_title,1, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
    ...
    $pdf->MultiCell(20, 0, ($sched->count > 0 ? $sched->time:'TBA'),1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
    $pdf->MultiCell(20, 0, ($sched->count > 0 ? $sched->room:'TBA'),1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
    $pdf->MultiCell(35, 0,  $instructor->lastname.', '.$instructor->firstname.' ',1, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
endforeach;
    ...
    $pdf->MultiCell(68, 0, '',0, 'C', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$lab = Modules::run('college/finance/getChargesByCategory',3, $student->semester, $student->school_year, $plan->fin_plan_id );
...
$totalFees = (($tuition->row()->amount*$totalUnits)+$misc->row()->amount+$lab->row()->amount) - $totalDiscount;
$prelim = $totalFees * .32;
...
$pdf->MultiCell(30, 0, (number_format($lab->row()->amount,2,'.',',')),1, 'R', 0, 0, '', '', true, 0, false, true, 0, 'M');
...
$pdf->MultiCell(30, 0, '',1, 'L', 0, 0, '', '', true, 0, false, true, 0, 'M');
