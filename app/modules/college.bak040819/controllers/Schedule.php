...
class schedule extends MX_Controller {
    ...
    {
        $instructor = $this->schedule_model->getInstructorPerSchedule($sched_code);
        return $instructor;
    }
    
        
    function loadSubject($value=NULL, $course_id=NULL)
    {
        $data['subjects'] = $this->schedule_model->searchSubject(urldecode($value), $course_id);
    ...
    {
        $data['id'] = $teacher_id;
        $data['subjects'] = $this->schedule_model->getSchedulePerTeacher($teacher_id);
    ...
    function getSchedulePerSection($section_id, $sem=NULL)
    {
        $schedule = $this->schedule_model->getSchedulePerSection($section_id, $sem);
        ...
        foreach ($schedule as $sched):
           $day_id[] = $sched->day_id;
           $time = date('g:i', strtotime($sched->t_from)).' - '.date('g:i', strtotime($sched->t_to));
        ...
        $result = json_encode(array('day' => $day, 'time' => $time,'room' => $room, 'sched_code' => $s_code,'count'=> $count));
        return $result;
    ...
    function time_range($lower = 7, $upper = 21, $step = .5, $format = NULL) {
    ...
    {
        $schedule = $this->schedule_model->getSingleTimeSchedule($day_id, $time, $sub_id, $sem);
        //return json_decode($schedule);
        return $schedule;
    }
    
            
    function getSingleTimeSchedulePerCourse($day_id, $course_id, $year, $time, $sem )
    {
        $schedule = $this->schedule_model->getTimeSchedPerCourse($course_id, $year, $day_id, $time, $sem);
        //print_r($schedule);
        return $schedule;
    }
            
    function getSingleTimeSchedulePerRoom($day_id, $room, $sem )
    {
        $schedule = $this->schedule_model->getTimeSchedPerRoom($day_id, $room, $sem);
        //print_r($schedule);
        return $schedule;
    }
    
    function getSchedulePerSubject($s_id=NULL, $sem=NULL)
    {
        $data['course'] = Modules::run('coursemanagement/getCourses');
        $data['collegeSubjects'] = Modules::run('college/subjectmanagement/getCollegeSubjects', $sem);
    ...
    function perCourse($course_id=NULL, $year=NULL, $sem=NULL)
    {
        $data['course'] = Modules::run('coursemanagement/getCourses');
        $data['collegeSubjects'] = Modules::run('college/subjectmanagement/getCollegeSubjects');
        $data['rooms'] = $this->getRoomsRaw();
    ...
    function addCollegeSched()
    {
        ...
        if($mon!=""):
            $item = explode(',', $mon);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 1, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
               $result = $this->saveSchedule($gcode,1, $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        
        if($tue!=""):
            $item = explode(',', $tue);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 2, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 2,  $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        if($wed!=""):
            $item = explode(',', $wed);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 3, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 3,  $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        if($thu!=""):
            $item = explode(',', $thu);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 4, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 4, $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        if($fri!=""):
            $item = explode(',', $fri);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 5, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 5, $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        if($sat!=""):
            $item = explode(',', $sat);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 6, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 6, $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        if($sun!=""):
            $item = explode(',', $sun);
            $conflict = $this->schedule_model->checkSched($room_id, $item[0], $item[1], 7, $spc_id, $fac_id);
            $ret = json_decode($conflict);
            if($ret->status):
                $result = $this->saveSchedule($gcode, 7, $room_id, $fac_id, $item[0], $item[1], $sub_id, $spc_id, $sem_id, $yearLevel,$color_code, $sec_id);
            endif;
        endif;
        
        if($ret->status):
            echo json_encode(array('msg'=>'Successfully Saved '.$ret->time));
        else:
            echo json_encode(array('msg'=>'Sorry, This schedule cannot be included in the system. There is a possible conflict of the schedule. Please check room and time availibilty carefully. '. 'source ='. $ret->time));
        endif;
    }
    
    function saveSchedule($gcode, $day_id, $room_id, $fac_id, $from, $to, $sub_id, $spc_id, $sem_id, $yearLevel, $color_code, $section_id = NULL)
    {
        
        $sched = $this->schedule_model->saveSchedule($gcode, $from, $to, $day_id, $room_id, $spc_id, $color_code, $section_id, $fac_id);
    ...
    {
        $result = $this->schedule_model->deleteSchedule($gcode);
        if($result):
