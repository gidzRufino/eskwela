...
<div class="col-lg-6">
    <table class="table table-stripped table-hover">
        ...
        <?php if($this->session->userdata('position_id') == 1 || $this->session->userdata('position_id') >= 65 && $this->session->userdata('position_id') <= 69 || $this->session->userdata('position_id')== 44 || $this->session->userdata('position_id')== 49 ): ?>
        <tfoot>
            <tr>
                <?php if($count!=$aprv): ?>
                <td colspan="4">
...
<div id="printStudyLoad"  style="width:35%; margin: 50px auto;"  class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="panel panel-green" style='width:100%;'>
        ...
        <div class="panel-body">
            ...
            <a href='#' id='<?php echo $g->grade_id ?>' data-dismiss='modal' onclick='printStudyLoad(<?php echo $student->semester; ?>)' style='margin-right:10px; color: white;' class='btn btn-xs btn-success pull-left'>Yes</a>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('#btnApprove').click(function(){
            var url = "<?php echo base_url().'college/subjectmanagement/approveLoad/'?>"+'<?php echo $student->admission_id ?>'; // the script where you handle the form input.
    ...
    function printStudyLoad(sem)
    {
        if($('#reprint').is(':checked')){
           var url = "<?php echo base_url('college/subjectmanagement/printStudyLoad/').base64_encode($student->st_id)?>/"+sem+'/'+1;
        }else{
            var url = "<?php echo base_url('college/subjectmanagement/printStudyLoad/').base64_encode($student->st_id)?>/"+sem;
        }
    ...
    function searchSubject(value)
     {
        ...
        var url = "<?php echo base_url().'college/subjectmanagement/loadSubject/'?>"+value+'/'+course_id+'/'+sem; // the script where you handle the form input.
    ...
    function saveLoadedSubject(sub_id, section_id, overided)
    {
        var url = "<?php echo base_url().'college/subjectmanagement/saveLoadedSubject/' ?>";
         $.ajax({
               ...
               url: url,
               //dataType: 'json',
               data: "subject_id="+sub_id+'&section_id='+section_id+'&overide='+overided+'&admission_id='+'<?php echo $student->admission_id ?>'+'&student_id='+'<?php echo $student->u_id ?>'+'&csrf_test_name='+$.cookie('csrf_cookie_name')                                                                                                                                                                                                                                                                                     , // serializes the form's elements.
               success: function(data)
               {
    ...
    function removeLoadedSubject(sub_id)
    {
        var url = "<?php echo base_url().'college/subjectmanagement/removeLoadedSubject/' ?>";
         $.ajax({
               ...
               url: url,
               dataType: 'json',
               data: "subject_id="+sub_id+'&admission_id='+'<?php echo $student->admission_id ?>'+'&student_id='+'<?php echo $student->u_id ?>'+'&csrf_test_name='+$.cookie('csrf_cookie_name')                                                                                                                                                                                                                                                                                     , // serializes the form's elements.
    ...
    function removeSubject(subject_id, pre_req)
    {
        var lectUnit = $('#'+subject_id+'_lect').html();
        var unitTaken = $('#totalLect_taken').html();
        removeLoadedSubject(subject_id);
        var unitsTaken = parseInt(unitTaken) - parseInt(lectUnit);
        $('#totalLect_taken').html(unitsTaken)
