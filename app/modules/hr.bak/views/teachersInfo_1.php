...
<div class="well col-lg-12">
    <div class="col-lg-2">
        <div rel="clickover" 
             data-content='<?php echo Modules::run('main/showUploadForm', $basicInfo->uid) ?>'
                 class="clickover">
    ...
    <div class="tab-content col-lg-12">
        <div style="padding-top: 15px;" class="tab-pane active" id="PersonalInfo">
                <div class="col-lg-6 pull-right">
                    <img class="img-circle img-responsive pull-right" style="width:150px; border:5px solid #fff" src="<?php if($basicInfo->user_id!=""):echo base_url().'uploads/sign/'.$basicInfo->user_id.'.bmp';else:echo base_url().'uploads/noImage.png'; endif;  ?>" />
                </div>
            <div class="col-lg-6">
                ...
                <dl class="dl-horizontal">
                    ...
                    <dd>
                        <span onclick="$('#bdate').datepicker()" title="double click to edit" id="a_bdate" ondblclick="$('#a_bdate').hide(), $('#bdate').show(),$('#bdate').focus()">
                            <?php echo $basicInfo->cal_date; ?>
                        </span> 
                        <input style="display: none;" name="inputBdate" type="text" data-date-format="mm-dd-yyyy" id="bdate" value="<?php echo $basicInfo->cal_date; ?>" placeholder="Date of Birth" 
        ...
        <div style="padding-top: 15px;" class="tab-pane" id="statutory">
            <?php
                $data['salaryGrade'] = Modules::run('hr/getSalaryGrade');
                $data['basicInfo'] = $basicInfo;
        ...
        <div style="padding-top: 15px;" class="tab-pane" id="od_info">
            <?php
                $data['basicInfo'] = $basicInfo;
                $data['deductions'] = Modules::run('hr/getOD_list');
                $data['myLoans'] = Modules::run('hr/getPersonalLoanRequest', $this->session->userdata('employee_id'));
...
<script type="text/javascript">
    ...
    function editAddressInfo()
    {
        ...
        $.ajax({
               ...
               url: url,
               //dataType: 'json',
               data: 'street='+$('#street').val()+'&barangay='+$('#barangay').val()+'&city='+$('#city').val()+'&province='+$('#province').val()+'&address_id='+$('#address_id').val()+'&zip_code='+$('#zip_code').val()+'&csrf_test_name='+$.cookie('csrf_cookie_name'), // serializes the form's elements.
               success: function(data)
               {
                   //$("form#quoteForm")[0].reset()
