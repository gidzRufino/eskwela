...
foreach($students->result() as $mStudent):
    ...
    if($settings->set_logo!='noImage.png'):
        $pdf->Image($image_file, $x, $y, 20, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
    else:
    ...
    foreach($subject_ids as $sub):
          $singleSub = Modules::run('academic/getSpecificSubjects', $sub->sub_id);
            ...
            $pdf->SetX($x+2);
            if($singleSub->parent_subject!=18):
                $pdf->MultiCell(33, 5,'     '.$singleSub->short_code,1, 'L', 0, 0, '', '', true, 0, false, true, 5, 'T');
            else:
                $m++;
                if($m==1):
                    $pdf->MultiCell(33, 5, "     MAPEH",1, 'L', 0, 0, '', '', true, 0, false, true, 5, 'T');
        ...
        endforeach;
        // add the average grade of MAPEH
        $genAve += $fg;
        $sum =
        $pdf->Ln();
        $pdf->SetX($x+2);
        ...
        $pdf->Ln();
        $pdf->SetX($x+2);
        $pdf->MultiCell(33, 5, 'GENERAL AVERAGE',1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'T', true);
        $pdf->MultiCell(33, 5, round($genAve/($numSubs+1), 3),1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'T');
    ...
    $bhRating = Modules::run('gradingsystem/getBHRating', $mStudent->st_id, segment_5, $sy);
    foreach ($bhRating->result() as $bh):
        $trans = Modules::run('gradingsystem/new_gs/getNumericalTransmutation', $bh->bh_id, $bh->rate);
        $total += $trans->trans_info;

    endforeach;
        $trans = Modules::run('gradingsystem/new_gs/getNumericalTransmutation', 0, $total);

        $conducts = Modules::run('reports/getaconduct', $mStudent->st_id);
        switch ($conducts->row()->$q) {
              case 1:
                $c1 = "U";
                break;
              case 2:
        ...
        $pdf->Ln();
        $pdf->SetX($x+2);
        $pdf->MultiCell(99, 5, "",1, 'L', 0, 0, '', '', true, 0, false, true, 5, 'T');
        $pdf->Ln();
        $pdf->SetX($x+2);
        $pdf->MultiCell(33, 5, "Conduct Grade",1, 'L', 0, 0, '', '', true, 0, false, true, 5, 'T');
        $pdf->MultiCell(33, 5, $c1,1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'T');
        $pdf->MultiCell(33, 5, "",1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'T');
