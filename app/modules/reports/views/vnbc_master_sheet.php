    ...
    foreach($subject_ids as $s){  
        ...
        if($singleSub->parent_subject==11):
            ...
            if($mapeh==4):
                ...
                $pdf->MultiCell(18, 3,'MAPEH',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
            endif;
        else:
...
foreach($male->result() as $s){
        ...
        $msubs++;
          ...
          $finalGrade = Modules::run('gradingsystem/getFinalGrade', $s->st_id, $singleSub->subject_id, segment_4, segment_5);
            if($singleSub->parent_subject==11):
                $mp++;
                if($mp==4):
                    $pdf->MultiCell(18, 3, round($grd/4, 2, PHP_ROUND_HALF_UP),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                endif;
            else:
                if($finalGrade->row()->final_rating!=0):
                    $pdf->MultiCell(18, 3,$finalGrade->row()->final_rating,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                else:
    ...
    unset($grd);
    $mavg = round($finalAssessment/count($subject_ids), 3);
    $pdf->MultiCell(5, 3, '',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, $mavg,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if($mavg >=75):
...
foreach($female->result() as $fem){
    ...
    foreach($subject_ids as $sub){  
          ...
          $finalGrade = Modules::run('gradingsystem/getFinalGrade', $fem->st_id, $singleSub->subject_id, segment_4, segment_5);
            if($singleSub->parent_subject==11):
                $fgrd += $finalGrade->row()->final_rating;
            endif;
    ...
    foreach($subject_ids as $sub){  
          ...
          $finalGrade = Modules::run('gradingsystem/getFinalGrade', $fem->st_id, $singleSub->subject_id, segment_4, segment_5);
            ...
            if($singleSub->parent_subject==11):
                $mp++;
                if($mp==4):
                    $pdf->MultiCell(18, 3, round($fgrd/4, 2, PHP_ROUND_HALF_UP),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
                endif;
            else:
                if($finalGrade->row()->final_rating!=0):
                    $pdf->MultiCell(18, 3,$finalGrade->row()->final_rating,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    ...
    unset($fgrd);
    $favg = round($finalAssessment/count($subject_ids), 3);
    $pdf->MultiCell(5, 3, '',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, $favg,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if($favg >=75):
        $pdf->MultiCell(35, 3, 'PASSED',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    else:
        $pdf->MultiCell(35, 3, 'FAILED',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
