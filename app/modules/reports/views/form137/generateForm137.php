        ...
        <h3 class="page-header" style="margin-top:5px; margin-bottom: 5px;">Generate DepEd Form 137 - A
            <div class="form-group pull-right" style="margin-right:20px;font-size: 15px;">
                    <div class="controls" id="AddedSection">
                        ...
                        <a class="text-warning" onclick="document.location='<?php echo base_url('reports/reports_f137/getNewInfo') ?>'">
     ...
     <div class="panel panel-yellow">
        <div class="panel-heading">
         ...
         <div class="panel-body">
            ...
            echo form_open_multipart(base_url().'reports/reports_f137/uploadF137', $attributes);
            ?>
...
<script type="text/javascript">
    ...
    {
      var url = '<?php echo base_url().'reports/reports_f137/exportStudentListToExcell/' ?>'+section;
    ...
    {
        var url = "<?php echo base_url().'reports/saveAcademicRecords/'?>"+$('#user_id').val()
        $.ajax({
            type: "POST",
            url: url,
            data:'csrf_test_name='+$.cookie('csrf_cookie_name')+'&school_year='+$('#form137School_year').val()
                  +'&school='+$('#school').val()+'&first='+$('#first').val()+'&second='+$('#second').val()
                  +'&third='+$('#third').val()+'&fourth='+$('#fourth').val()+'&average='+$('#average').val()+'&generalAverage='+$('#generalAverage').val()+'&subject_id='+$('#inputSubject').val()+'&grade_id='+$('#selectedLevel').val(),
            success: function(data)
            {
