...
class hr extends MX_Controller {
    ...
    function editHrTime()
    {
        ...
        $timeUnits = explode('-', $this->input->post('time_option'));
        $column = $timeUnits[1];
        
        if($this->hr_model->editHrTime($column, $att_id, $hour, $min)):
            echo 'Success';
        else:
    ...
    function getAllEmployee($page=null)
    {
            $this->load->library('table');

            $result = $this->hr_model->getAllEmployee('','');
            $config['base_url'] = base_url('hr/getAllEmployee');
            $config['total_rows'] = $result->num_rows();
            $config['per_page'] = 10;
            $config['num_links'] = 5;
            $config['full_tag_open'] = '<ul class="pagination">';
            ...
            $this->pagination->initialize($config);
            $page = $this->hr_model->getAllEmployee($config['per_page'], $page);
