...
foreach ($subject_ids as $sp):
    ...
    $finalGrade2 = Modules::run('gradingsystem/getFinalGrade', $mStudent->st_id, $singleSub->subject_id, 2, $sy);
    $finalGrade3 = Modules::run('gradingsystem/getFinalGrade', $mStudent->st_id, $singleSub->subject_id, 3, $sy);
    $finalGrade4 = Modules::run('gradingsystem/getFinalGrade', $mStudent->st_id, $singleSub->subject_id, 4, $sy);
    if ($singleSub->parent_subject == 11):
        $grd += $finalGrade->row()->final_rating;
        $grd2 += $finalGrade2->row()->final_rating;
        $grd3 += $finalGrade3->row()->final_rating;
        $grd4 += $finalGrade4->row()->final_rating;
        $mp += 1;
    endif;
endforeach;
$fg = round($grd / $mp);
...
foreach ($subject_ids as $sub):
    ...
    if ($singleSub->parent_subject != 11):
        ...
        $pdf->MultiCell(6, 5, ($finalGrade2->row()->final_rating != 0 ? $finalGrade2->row()->final_rating : ''), '1', 'C', (($z % 2) == 0 ? 0 : 1), 0, '', '', true, 0, false, true, 5, 'M');
        $pdf->MultiCell(6, 5, ($finalGrade3->row()->final_rating != 0 ? $finalGrade3->row()->final_rating : ''), '1', 'C', (($z % 2) == 0 ? 0 : 1), 0, '', '', true, 0, false, true, 5, 'M');
        $pdf->MultiCell(6, 5, ($finalGrade4->row()->final_rating != 0 ? $finalGrade4->row()->final_rating : ''), '1', 'C', (($z % 2) == 0 ? 0 : 1), 0, '', '', true, 0, false, true, 5, 'M');
        ...
        if ($m == 1):
            ...
            $pdf->MultiCell(6, 5, ($fg2 == 0 ? '' : $fg2), '1', 'C', (($z % 2) == 0 ? 1 : 0), 0, '', '', true, 0, false, true, 5, 'M');
            $pdf->MultiCell(6, 5, ($fg3 == 0 ? '' : $fg3), '1', 'C', (($z % 2) == 0 ? 1 : 0), 0, '', '', true, 0, false, true, 5, 'M');
            $pdf->MultiCell(6, 5, ($fg4 == 0 ? '' : $fg4), '1', 'C', (($z % 2) == 0 ? 1 : 0), 0, '', '', true, 0, false, true, 5, 'M');
...
foreach ($deportment as $dept):
    ...
    if ($dept->bh_group == 1) {
        ...
        foreach ($sub as $bus):
            ...
            for ($termLoop = 0; $termLoop < 4; $termLoop++):
                $finalDeport2 = Modules::run('gradingsystem/gradingsystem_reports/getFinalBHRate', $mStudent->st_id, $bus->bh_id, $termLoop+1, $sy);
...
$pdf->MultiCell(6, 5, '11', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->MultiCell(6, 5, '0', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->MultiCell(6, 5, '0', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
//$totalDays = $sd->row()->July + $sd->row()->August + $sd->row()->September + $sd->row()->October + $sd->row()->November + $sd->row()->December + $sd->row()->January + $sd->row()->February + $sd->row()->March + $sd->row()->April;
$pdf->MultiCell(10, 5, '149', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
$pdf->Ln();
...
$theMonth = $attendance->July;
$theMonth7 = ($theMonth > 16?'16':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth7 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 8);
//$theMonth = $attendance->August + $holiday->num_rows();
$theMonth = $attendance->August;
$theMonth8 = ($theMonth > 23?'23':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth8 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 9);
//$theMonth = $attendance->September + $holiday->num_rows();
$theMonth = $attendance->September;
$theMonth9 = ($theMonth > 21?'21':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth9 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 10);
//$theMonth = $attendance->October + $holiday->num_rows();
$theMonth = $attendance->October;
$theMonth10 = ($theMonth > 23?'23':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ?  $theMonth10 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 11);
//$theMonth = $attendance->November + $holiday->num_rows();
$theMonth = $attendance->November ;
$theMonth11 = ($theMonth > 22?'22':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth11 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 12);
//$theMonth = $attendance->December + $holiday->num_rows();
$theMonth = $attendance->December;
$theMonth12 = ($theMonth > 15?'15':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth12 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 1);
//$theMonth = $attendance->January + $holiday->num_rows();
$theMonth = $attendance->January;
$theMonth1 = ($theMonth > 18?'18':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth1 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 2);
//$theMonth = $attendance->February + $holiday->num_rows();
$theMonth = $attendance->February;
$theMonth2 = ($theMonth > 11?'11':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth2 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 3);
//$theMonth = $attendance->March + $holiday->num_rows();
$theMonth = $attendance->March;
$theMonth3 = ($theMonth > 21?'21':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth3 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');

$holiday = Modules::run('calendar/holidayExist', 4);
//$theMonth = $attendance->April + $holiday->num_rows();
$theMonth = $attendance->April;
$theMonth4 = ($theMonth > 22?'22':$theMonth);
$pdf->MultiCell(6, 5, ($attendance ? $theMonth4 : 0), 1, 'C', 1, 0, '', '', true, 0, false, true, 5, 'M');
$totalDaysPresent = ($attendance ? $theMonth7 + $theMonth8 + $theMonth9 + $theMonth10 + $theMonth11 + $theMonth12 + $theMonth1 + $theMonth2 + $theMonth3 + $theMonth4 : 0);
