...
<thead style="background: #E6EEEE;">
    <tr> 
        ...
        {
        ?>
            <th class="text-center"><?php echo $cat->category_name.'<br /> ( '.($cat->weight*100).'% )' ?></th>    
        <?php
        }
        ?>  
    ...
    <?php
        ...
        <td  class="text-center pointer"  data-target="#assess_details" data-toggle="modal" onclick="getIndividualAssessment('<?php echo $student->st_id ?>',<?php echo $subject_id ?>,<?php echo $k->code ?>, <?php echo $term ?>)" class="values" >
         <?php
          ...
          $numberOfAssessment = Modules::run('gradingsystem/getEachScoreByStudent', $student->st_id, $k->code,   $term, $subject_id, 1, $student->section_id);
          ...
          if($numberOfAssessment->num_rows()>0){
                ...
                //$ps = (($record/$tps)*100); //this is original zero based computation
                $ps = ((($record*50)/$tps)+50);
