...
class MYPDF extends Pdf {
	...
	public function Header() {
		// Logo
                ...
                $this->SetTitle($settings->short_name.' Grading Sheet ');
                $image_file = K_PATH_IMAGES.'/pilgrim.jpg';
                $this->Image($image_file, 60, 7, 18, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);

                $image_file = K_PATH_IMAGES.'/uccp.jpg';
                $this->Image($image_file, 140, 7, 15, '', 'JPG', '', 'T', false, 300, '', false, false, 0, false, false, false);
                $this->Ln(5);
...
foreach($male->result() as $s){
    ...
    $generalAverage = (($firstM->row()->final_rating<=70?70:$firstM->row()->final_rating)+($secondM->row()->final_rating<=70?70:$secondM->row()->final_rating)+($thirdM->row()->final_rating<=70?70:$thirdM->row()->final_rating)+($fourthM->row()->final_rating<=70?70:$fourthM->row()->final_rating))/4;
    if($fourthM->row()->final_rating!=''):
        $GA = round($generalAverage, 0);
    else:
        $GA = '';
    endif;
    $pdf->MultiCell(15, 3, ($firstM->row()->final_rating!=""?($firstM->row()->final_rating<=70?70:$firstM->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($secondM->row()->final_rating!=""?($secondM->row()->final_rating<=70?70:$secondM->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($thirdM->row()->final_rating!=""?($thirdM->row()->final_rating<=70?70:$thirdM->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($fourthM->row()->final_rating!=""?($fourthM->row()->final_rating<=70?70:$fourthM->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
...
foreach($female->result() as $s){
    ...
    $generalAverage = (($firstF->row()->final_rating<=70?70:$firstF->row()->final_rating)+($secondF->row()->final_rating<=70?70:$secondF->row()->final_rating)+($thirdF->row()->final_rating<=70?70:$thirdF->row()->final_rating)+($fourthF->row()->final_rating<=70?70:$fourthF->row()->final_rating))/4;
    ...
    $pdf->SetFont('helvetica', 'B', 8);
    $pdf->MultiCell(15, 3, ($firstF->row()->final_rating!=""?($firstF->row()->final_rating<=70?70:$firstF->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($secondF->row()->final_rating!=""?($secondF->row()->final_rating<=70?70:$secondF->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($thirdF->row()->final_rating!=""?($thirdF->row()->final_rating<=70?70:$thirdF->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, ($fourthF->row()->final_rating!=""?($fourthF->row()->final_rating<=70?70:$fourthF->row()->final_rating):""),1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    $pdf->MultiCell(15, 3, $GA,1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    if($GA >= 75):
        $pdf->MultiCell(50, 3, 'Passed',1, 'C', 0, 0, '', '', true, 0, false, true, 10, 'M');
    else:
