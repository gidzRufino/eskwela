...
class gradingsystem extends MX_Controller {
    ...
    function getbhTransmuted(){
        $data = $this->gradingsystem_model->getbhTransmuted();
        return $data;
    }
...
//        
    ...
    function gsView($sub_id = null, $sec_id = null, $term = null, $sy = null){
        //$data['record'] = $this->classRecordDetails($sub_id,$sec_id,$term,$sy);
        //$data['title'] = 'View Class Record';
        $data['getSubject'] = Modules::run('academic/mySubject', '', $this->session->userdata('school_year'));
    ...
    public function classRecordDetails($section_id, $subject_id, $term, $year=NULL,$level = null)
    {
        $settings = Modules::run('main/getSet');
    ...
    function classRecordDetailsPerSubPerlvl($section,$subject,$level,$term,$sy){
        $settings = Modules::run('main/getSet');
        $gs_settings = $this->gradingsystem_model->getSettings($sy);
        $data['gs_used'] = $gs_settings->gs_used;
    }
    ...
    public function getTotalScoreByStudent($student_id = null, $category_id = null, $term = NULL, $subject_id = NULL, $faculty_id = NULL, $section) {
        $gs_settings = $this->gradingsystem_model->getSettings($this->session->userdata('school_year'));
        if($gs_settings->customized):
            $score = Modules::run('gradingsystem/new_gs/getTotalScoreByStudent',$student_id, $category_id, $term, $subject_id);
        else:
            $score = $this->gradingsystem_model->getTotalScoreByStudent($student_id, $category_id, $term, $subject_id, $faculty_id,$section);
        endif;
...
//print_r($bh);
    ...
    function getFinalCardByLevel($st_id, $levelCode, $school_year=NULL){
        $final = $this->gradingsystem_model->getFinalCardByLevel(base64_decode($st_id), $levelCode, $school_year);
        if(!$final):
            return FALSE;
        else:
            return $final;
