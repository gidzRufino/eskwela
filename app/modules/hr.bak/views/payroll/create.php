    ...
    <div class="form-group pull-right">
        <select onclick="generatePayroll(this.value)" tabindex="-1" id="payPeriod" name="payPeriod" style="width:300px;">
           ...
           <?php foreach($payrollPeriod as $pp): ?>
                <option id="option_<?php echo $pp->per_id ?>" value="<?php echo $pp->per_id ?>" <?php echo ($pc_code==$pp->per_id) ? 'selected' : ''; ?>><?php echo date('F d, Y', strtotime($pp->per_from)).' - '.date('F d, Y', strtotime($pp->per_to)); ?></option>
           <?php endforeach; ?>
...
<div id="addCharges"  style="margin: 50px auto;"  class="modal fade col-lg-5" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="panel panel-success" style="margin:0; padding-bottom: 10px;">
        ...
        <form class="panel-body" id="updateDeductionForm">
            <div class="col-lg-12">
                <h4 class="text-center" >Gross Pay: <span class="text-danger" id="grossPay"></span></h4>
            </div>
            <hr class="col-lg-11 no-margin" />
            <h4 class="text-center no-margin">Deductions</h4>
            <hr class="col-lg-11 no-margin" />
...
<script type="text/javascript">
    ...
    function generatePayroll(pType)
    {
        ...
        var url = "<?php echo base_url().'hr/payroll/create/' ?>"+pType; // the script where you handle the form input.
