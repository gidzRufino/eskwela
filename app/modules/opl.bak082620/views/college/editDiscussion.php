...
<script>
    ...
    {
        var id = $(btn).attr('discussion-id'),
        subject = $(btn).attr('discussion-subject'),
        section = $(btn).attr('discussion-section'),
        base = $("#base").val(),
        ...
        $.ajax({
            ...
            success: function(data){
                alert(data.message);
                document.location = base + "opl/college/discussionBoard/<?php echo $this->session->school_year."/".$this->session->employee_id; ?>/"+section+"/"+subject+"/<?php echo $semester; ?>";
            }
        })
    }
	...
	function readyDelete(btn)
	{
		...
		$("#deleteDiscussion").find("#deleteBtn").attr('discussion-subject', subject);
		$("#deleteDiscussion").find("#deleteBtn").attr('discussion-section', section);
    ...
    function showEditDiscussion(btn)
    {
        ...
        $.ajax({
            url: "<?php echo site_url('opl/getDiscussionDetails/'); ?>"+$(btn).attr('discussion-id'),
            type: "GET",
            dataType: "JSON",
            success: function(data){
                modal.find("#discussDetails").summernote('code', data.dis_details);
            }
        })
