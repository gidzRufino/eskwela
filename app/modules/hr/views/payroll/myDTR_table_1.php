<table border="1" style="margin:0; border: 1px solid #DDDDDD;"  class="table">
    ...
    <table class='table table-striped'> 
        ...
        $officialTime = Modules::run('hr/hrdbprocess/getTimeShift', $info->shift_group_id);
...
//        $officialTimeOutPm = $officialTime->ps_to_pm;
        ...
        if($timeOutPMCompute!=0):
                $undertimePM =  (strtotime($officialTimeOutAm) - strtotime($time_out_pm)) <= 0?0:(strtotime($officialTimeOutAm) - strtotime($time_out_pm))/60;
       ...
       // $totalUndertimeTardy = ($tardyAm + $undertimeAm)+($tardyPm+$undertimePm);
        $totalUndertimeTardy = $totalUndertimePm+$tardyPm;
?>
        <tr>
            <td width="10%">
...
<input type="hidden" id="totalHoursRendered" value="<?php echo $totalHours ?>" />
