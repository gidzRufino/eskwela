...
<div class="row">
    <div class="col-lg-12">
        <h2 style="margin-top: 20px;" class="page-header">Admissions <small id="schoolID">( <?php echo $sy ?> ) <span style="cursor: pointer;" onclick="$('#schoolID').fadeOut(500), $('#inputLRN').fadeIn(500),$('#inputLRN').focus(),$('#inputLRN').val('')" >[Set Student ID Manually]</span></small>
    ...
    function selectDepartment(value)
    {
      ...
      function setId(levelCode){
      	...
      	var lname = $('#inputLastName').val();
              ...
              url = "<?php echo base_url().'registrar/getLatestIdNum/'?>"+levelCode+'/'+fname+'/'+lname; // the script where you handle the form input. 
            ...
            $.ajax({
                   ...
                   url: url,
                   dataType: 'json',
                   data: "level_id="+levelCode+'&fname='+fname+'&lname='+lname+'&csrf_test_name='+$.cookie('csrf_cookie_name'), // serializes the form's elements.
                   success: function(data)
                   {
                   	if(data.status == true) {
                       ...
                       if(id<10)
                           ...
                           }else
                           {
                            ...
                            $('#schoolID').html(<?php echo date('y', strtotime($sy)) ?>+'-UP-'+prefix+id);
                            $('#inputIdNum').val(<?php echo date('y', strtotime($sy)) ?>+'-UP-'+prefix+id); 
                   	} else {
                   		alert('Information Entered Already Exist');
                   		location.reload();
                   	}
