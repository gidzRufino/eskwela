    ...
    for($x=$firstDay;$x<=$lastDay;$x++){
        ...
        if($day=='Sat'||$day=='Sun')
        {
            ...
            if($this->session->userdata('attend_auto')):
                $ifPresent = Modules::run('attendance/ifPresent',$s->rfid, $x, segment_4, $year, 1);
            else:    
                $ifPresent = Modules::run('attendance/ifPresent',$s->st_id, $x, segment_4, $year, 0);
            endif;
            ...
            if($ifPresent):
                ...
                if($this->session->userdata('attend_auto')):
                    $remarks = Modules::run('attendance/getAttendanceRemark', $s->rfid,  $year.'-'.segment_4.'-'.$x);
                else:
            ...
            //lowerShades($pdf, 0, ((30.4)*0), 0, (30*0), 0, (30.4*0));
...
//absent
 $pdf->MultiCell(15, 15, $ma-$holiday->num_rows(),1, 'C', 0, 0, '', '', true, 0, false, true, 15, 'M');
//tardy 
  if($this->session->userdata('attend_auto')):
   $tardy = Modules::run('attendance/getTardy', $s->rfid, segment_4, segment_5); 
 else:
   $tardy = Modules::run('attendance/getTardy', $s->st_id, segment_4, segment_5);
 endif;
 $pdf->MultiCell(15, 15, $tardy->num_rows(),1, 'C', 0, 0, '', '', true, 0, false, true, 15, 'M');
 //remarks
 $rem = Modules::run('main/getAdmissionRemarks', $s->st_id, segment_4, $school_year);
 if($rem->num_rows()>0){
   ...
   $columnCount = 0;
    ...
    for($x=$firstDay;$x<=$lastDay;$x++){
        ...
        if($day=='Sat'||$day=='Sun')
        {
            ...
            $r = $z*($y-1); 
             if($this->session->userdata('attend_auto')):
                $ifPresent = Modules::run('attendance/ifPresent',$s->rfid, $x, segment_4, $year);
            else:    
...
//absent
 $pdf->MultiCell(15, 15, $fa-$holiday->num_rows(),1, 'C', 0, 0, '', '', true, 0, false, true, 15, 'M');
//tardy 
 if($this->session->userdata('attend_auto')):
   $tardy = Modules::run('attendance/getTardy', $s->rfid, segment_4, $year); 
 else:
       ...
       $pdf->MultiCell(50, 12, '',1, 'C', 0, 0, '', '', true, 0, false, true, 15, 'M');
        $columnCount = 0;
         ...
         for($x=$firstDay;$x<=$lastDay;$x++){
             ...
             $day = date('D', strtotime(segment_5.'-'.segment_4.'-'.$x));
