...
//        return;
    ...
    function chooseOcc($data, $id, $school_year = NULL, $mf) {
        $this->db = ($school_year == NULL ? $this->eskwela->db($this->session->school_year) : $this->eskwela->db($school_year));
        $this->db->select('*');
        $this->db->from('profile_occupation');
        ...
        if ($query->num_rows() > 0):
            $occ_id = $query->row()->occ_id;
            $this->setUpdateOccupation($occ_id, $id, $school_year, $mf);
        else:
            $this->setOccupation($data, $id, $school_year, $mf);
        endif;
    }
    ...
    function setUpdateOccupation($occ_id, $id, $school_year = NULL, $mf) {
        $this->db = ($school_year == NULL ? $this->eskwela->db($this->session->school_year) : $this->eskwela->db($school_year));
        $data = array(
            $mf . '_occ' => $occ_id
        );
    ...
    function editparentinfo($user_id, $details, $school_year = NULL) {
        ...
        $this->db->update('profile_parent', $details);
...
//        $query = $this->db->get();
    ...
    function saveProfileParent($mf, $profile_id, $school_year) {
        $this->db = ($school_year == NULL ? $this->eskwela->db($this->session->school_year) : $this->eskwela->db($school_year));
        $this->db->where('u_id', $profile_id);
        $q = $this->db->get('profile_parent');

        if ($q->num_rows() > 0):
            $this->db->where('u_id', $profile_id);
            $this->db->update('profile_parent', $mf);
        else:
            $this->db->insert('profile_parent', $mf);
        endif;
    }
