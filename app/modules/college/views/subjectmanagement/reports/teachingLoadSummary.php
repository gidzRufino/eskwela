...
foreach ($schedule as $sched):
    ...
    $pdf->MultiCell(25, 5, 'Subject', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
    ...
    $pdf->MultiCell(40, 5, 'Schedule', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
    $pdf->MultiCell(15, 5, 'Unit/s', 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
    $pdf->Ln();

    $subjects = Modules::run('college/subjectmanagement/getAssignSubject', $sched->school_year, $sched->semester, base64_encode($employee->employee_id));
    $totalUnits = 0;
    $x = 0;
    foreach ($subjects as $s):
        ...
        $scheds = Modules::run('college/schedule/getSchedulePerSection', $s->sec_id);
        $sched = json_decode($scheds);

        $pdf->SetX(5);
        $pdf->MultiCell(8, 5, $x, 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
        $pdf->MultiCell(25, 5, $s->sub_code, 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
        ...
        $pdf->MultiCell(40, 5, ($sched->count > 0 ? $sched->time . ' [ ' . $sched->day . ' ]' : ''), 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
        $pdf->MultiCell(15, 5, ($s->sub_code == "NSTP 11" || $s->sub_code == "NSTP 12" || $s->sub_code == "NSTP 1" || $s->sub_code == "NSTP 2" ? 3 : ($s->s_lect_unit + $s->s_lab_unit)), 1, 'C', 0, 0, '', '', true, 0, false, true, 5, 'M');
        $pdf->Ln();
        switch ($pdf->PageNo()):
            case 1:
                if ($tcount == 38):
