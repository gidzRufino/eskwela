        ...
        <h5>Learner's Observed Values
            <select class="pull-right">
                ...
                <?php foreach ($core_val as $cv): ?>
                    <option value="<?php echo $cv->core_id ?>" onclick="getBHrate(this.value)"><?php echo $cv->core_values ?></option>
                <?php endforeach; ?>
            </select>
        </h5>
        <hr />
...
//        alert(url);
    ...
    function getBHrate(id) {
        ...
        $.ajax({
            type: 'POST',
            data: 'id=' + id + '&short_name=' + school_name + '&stid=' + st_id + '&term=' + term + '&sy=' + sy + '&csrf_test_name=' + $.cookie('csrf_cookie_name'),
            url: url,
            success: function (data) {
                $('#cv').html(data);
            }
